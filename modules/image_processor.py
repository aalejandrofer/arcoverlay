...
class ImageProcessor:
    ...
    def find_color_region(img: Image.Image, target_color: Tuple[int, int, int], tolerance: int = 40) -> Optional[Tuple[int, int, int, int]]:
        # Direct processing without downscaling (As requested)
        img_array = np.array(img.convert('RGB'))
        r, g, b = target_color
        
        # Check if pixels are within tolerance of the target color
        ...
        try:
            ...
            if debug_path and debug_prefix:
                try:
                    ...
                    search_img.save(os.path.join(debug_path, raw_filename))
